...
class SinaExtractorNewGame(BaseExtractor):
 
    def __init__(self, config):
        super(SinaExtractorNewGame, self).__init__(config)
...
#             driver = webdriver.Firefox()
            ...
            link_list = driver.find_element_by_class_name("list").find_elements_by_class_name("item")
            print len(link_list)
    
            for elem in link_list:
                title = elem.find_element_by_tag_name("h1").find_element_by_tag_name("a").text # article title
                if title not in list:
                    LOGGER.debug("article title %s"%(title))
                    url = elem.find_element_by_tag_name("h1").find_element_by_tag_name("a").get_attribute("href")
